...
for mi = 1:nM
    for ni = 1:nNI
        ...
        for ci = 1:nCP
            ...
            if ci == 1
                title('Raw', 'Color','w', 'FontSize',14, 'FontWeight','normal');
            end
            hold off;
        ...
        if doExport && ~isempty(outDir) && isfolder(outDir)
            ...
            fprintf('  Exported: %s\n', fn_heat);
            fprintf('  Exported: %s\n', fn_labs);
            fn_tab = fullfile(outDir, ...
                sprintf('cellpose_sweep_table_%s.tex', safeMTag));
            writeCPtableLatex(fn_tab, cpModels{mi}, niStr, cpVals, ftVals, ...
                              nObj_mn, cov_mn, cpMinSize);
            fprintf('  Exported: %s\n', fn_tab);
        elseif doExport
...
% writeCPtableLatex  Write a LaTeX tabular of sweep results to a .tex file.
%   The file can be \input{} into BlobFilters_manual.tex via an \IfFileExists
%   guard.  Each cell contains n_objects / coverage%.
nCP = numel(cpVals);
nFT = numel(ftVals);
...
fprintf(fid, '%% Format per cell: n_objects / coverage%%\n\n');

% Column spec: first col right-aligned, then one right-aligned col per ft value
fprintf(fid, '\\begin{tabular}{r|%s}\n', repmat('r', 1, nFT));
fprintf(fid, '\\hline\n');

% Header row: ft values
fprintf(fid, '\\texttt{cp} $\\backslash$ \\texttt{ft}');
for fi = 1:nFT
    fprintf(fid, ' & \\texttt{%.2f}', ftVals(fi));
end
fprintf(fid, ' \\\\\n\\hline\n');
